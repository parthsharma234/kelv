...
const VoiceTimeline: React.FC<VoiceTimelineProps> = ({ voiceTimeline, voiceRecommendations }) => {
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {METRICS.map(m => {
                  const currentVal = selectedPoint.metrics[m.key as keyof typeof yRanges];
                  const prevPoint = voiceTimeline[voiceTimeline.findIndex(pt => pt.timestamp === selectedPoint.timestamp) - 1];
                  const prevVal = prevPoint ? prevPoint.metrics[m.key as keyof typeof yRanges] : null;
                  ...
                  return (
                    <div key={m.key} className="flex justify-between items-center">
                      ...
                      <span className="text-white">
                        {Math.round(currentVal)} WPM
                        {delta !== null && (
                          <span className={`ml-2 text-sm ${delta > 0 ? 'text-green-400' : delta < 0 ? 'text-red-400' : 'text-gray-400'}`}>
                            {delta > 0 ? '+' : ''}{delta}
                          </span>
                        )}
                      </span>
      ...
      <div className="bg-dark-800 rounded-2xl p-6">
        ...
        {voiceRecommendations ? (
          <div className="space-y-4">
            {voiceRecommendations.summary && (
              <p className="text-gray-300 mb-2">{voiceRecommendations.summary}</p>
            )}
            {voiceRecommendations.strengths && voiceRecommendations.strengths.length > 0 && (
              <div>
                <h5 className="text-green-400 font-medium mb-3">Key Strengths to Maintain:</h5>
                <div className="space-y-2">
                  {voiceRecommendations.strengths.slice(0, 5).map((strength, index) => (
            ...
            {voiceRecommendations.priorityImprovements && voiceRecommendations.priorityImprovements.length > 0 && (
              <div>
                <h5 className="text-blue-400 font-medium mb-3">Priority Improvements:</h5>
                <div className="space-y-2">
                  {voiceRecommendations.priorityImprovements.slice(0, 5).map((tip, index) => (
                    <div key={index} className="flex items-start gap-2">
            ...
            {voiceRecommendations.specificTips && voiceRecommendations.specificTips.length > 0 && (
              <div>
                <h5 className="text-blue-400 font-medium mb-3">Specific Tips:</h5>
                <div className="space-y-2">
                  {voiceRecommendations.specificTips.slice(0, 5).map((tip, index) => (
                    <div key={index} className="flex items-start gap-2">
                      <Info className="w-4 h-4 text-blue-400 mt-0.5 flex-shrink-0" />
                      <span className="text-sm text-gray-300">{tip}</span>
                    </div>
